...
def render_selector_preciario():
    ...
    if modalidad_cotizacion == "Cotización Directa (Preciario / Manual)":
        with st.container(border=True):
            ...
            # --- NUEVO BOTÓN GRANDE PARA ELEGIR EL ORIGEN ---
            origen_captura = st.radio(
                "📂 ¿Cómo deseas agregar el concepto?",
                ["Buscar en Preciario BESCO (Google Sheets)", "Captura Manual Libre"],
                horizontal=True
            )
            usar_preciario_besco = (origen_captura == "Buscar en Preciario BESCO (Google Sheets)")
            ...
            if usar_preciario_besco:
                try:
                    ...
                    st.info("Se habilitará el modo de captura manual libre automáticamente.")
                    usar_preciario_besco = False
                    origen_concepto = "Captura manual"

            if not usar_preciario_besco:
                origen_concepto = "Captura manual"
                st.info("Modo de captura manual libre habilitado.")
