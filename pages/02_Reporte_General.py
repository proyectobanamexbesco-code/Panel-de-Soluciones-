...
# Consideramos las rutas donde pueda estar el logo (buscando primero el nuevo logo besco 2026.jpeg)
...
st.set_page_config(page_title="BESCO | Evidencia Técnica", page_icon="📑", layout="wide")
...
class BESCO_PDF(FPDF):
    ...
    def header(self):
        # Insertar Logo con el tamaño aumentado (+50%)
        if LOGO_PATH and os.path.exists(LOGO_PATH):
            try:
                # El tamaño original era w=final_w, h=25. Lo aumentamos a h=38 aprox (+50%)
                ...
                final_h = 38  # 50% más grande que el original 25
        ...
        self.set_xy(100, 15)
        # Título principal del reporte (sin la palabra "besco" flotando como en la cotización)
        self.cell(0, 10, limpiar_texto_pdf('REPORTE DE SERVICIO TÉCNICO - BESCO'), 0, 1, 'R')
        self.set_font('Arial', '', 9)
        self.set_x(100)
        self.cell(0, 5, limpiar_texto_pdf(f"Emisión del Reporte: {datetime.now().strftime('%d/%m/%Y %H:%M')}"), 0, 1, 'R')
        self.ln(15) # Ajuste de salto de línea por el logo más grande
    ...
    def add_custom_section(self, title):
        ...
        self.cell(0, 8, limpiar_texto_pdf(f"{self.section_count}. {title.upper()}"), 0, 1, 'L', fill=True)
        self.section_count += 1
        self.ln(2)
    ...
    def photo_grid(self, title, photos, eq_index=0, prefix="img"):
        ...
        for i, foto in enumerate(photos):
            ...
            if col == 0 and (self.get_y() + alto_foto > 265):
                ...
                self.cell(0, 6, limpiar_texto_pdf(f"(Continuación) {title}"), 0, 1, 'L')
                self.set_text_color(0, 0, 0)
                self.ln(2)
...
with st.container(border=True):
    ...
    # Limite de 20 caracteres para folio, como en cotizaciones
    ...
    # Lista de oficinas incluyendo Tampico, como se solicitó previamente
    lista_oficinas = [
...
with st.container(border=True):
    ...
    for i in range(num_equipos):
        with st.expander(f"⚙️ CONFIGURACIÓN EQUIPO {i+1}", expanded=(i==0)): # Solo expande el primero por defecto
            ...
            fa = col_f1.file_uploader("Fotos ANTES (Máx 4 recomendadas)", accept_multiple_files=True, key=f"fa_{i}", type=["jpg", "jpeg", "png"])
            fd = col_f2.file_uploader("Fotos DESPUÉS (Máx 4 recomendadas)", accept_multiple_files=True, key=f"fd_{i}", type=["jpg", "jpeg", "png"])
...
with st.container(border=True):
    ...
    df_mat = st.data_editor(pd.DataFrame(columns=["Cantidad", "Descripción"]), num_rows="dynamic", use_container_width=True)
...
with st.container(border=True):
    ...
    mapeo_correos = {
        ...
        # Correos obligatorios para Tampico integrados
    ...
    if st.button("🚀 Generar y Enviar Reporte Final", type="primary", use_container_width=True):
        if not cliente or not folio:
            st.error("⚠️ El Cliente y el Folio/OT/TK son obligatorios para generar el reporte.")
        else:
            with st.spinner("Construyendo documento PDF y procesando imágenes..."):
                ...
                pdf.cell(0, 7, limpiar_texto_pdf(f"Cliente: {cliente} | Folio: {folio}"), 0, 1)
                f_ejec_str = fecha_ejecucion.strftime('%d/%m/%Y')
                pdf.cell(0, 7, limpiar_texto_pdf(f"Fecha de Ejecución: {f_ejec_str} | Oficina: {oficina}"), 0, 1)
                if sucursal: pdf.cell(0, 7, limpiar_texto_pdf(f"Sucursal: {sucursal}"), 0, 1)
                pdf.set_font('Arial', 'B', 10)
                pdf.cell(0, 7, limpiar_texto_pdf(f"Técnico: {tecnico} | Supervisor: {supervisor}"), 0, 1)
                pdf.set_font('Arial', '', 10)
                pdf.cell(0, 7, limpiar_texto_pdf(f"Servicio: {tipo_serv} ({referencia})"), 0, 1); pdf.ln(5)
                ...
                for eq in equipos_data:
                    ...
                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(0, 7, limpiar_texto_pdf(f"Estatus Final: {eq['estatus']}"), 0, 1)
                    pdf.set_font('Arial', '', 10)
                    
                    valid_meds = {k: v for k, v in eq['meds'].items() if v}
                    for k, v in valid_meds.items(): 
                        pdf.cell(60, 6, limpiar_texto_pdf(f"{k}:"), 1)
                        pdf.cell(130, 6, limpiar_texto_pdf(f"{v}"), 1, 1)
                    if eq['otros']: 
                        pdf.multi_cell(0, 6, limpiar_texto_pdf(f"Detalles: {eq['otros']}"), 1)
                        
                    if eq['tag'] or eq['marca'] or eq['cap']: 
                        pdf.set_font('Arial', 'B', 9)
                        pdf.cell(0, 7, limpiar_texto_pdf(f"TAG: {eq['tag']} | Marca: {eq['marca']} | Cap: {eq['cap']}"), 0, 1)
                        pdf.set_font('Arial', '', 10)
                    
                    if eq['actividades']:
                        pdf.multi_cell(0, 6, limpiar_texto_pdf(f"Actividades Realizadas:\n{eq['actividades']}"), 1)
                    if eq['com']: 
                        pdf.multi_cell(0, 6, limpiar_texto_pdf(f"Comentarios Extras:\n{eq['com']}"), 1)
                ...
                if not df_c.empty:
                    ...
                    pdf.cell(160, 7, limpiar_texto_pdf("DESCRIPCIÓN"), 1, 1, 'C')
                    pdf.set_font('Arial', '', 9)
                    for _, row in df_c.iterrows(): 
                        pdf.cell(30, 7, str(row["Cantidad"]), 1)
                        pdf.cell(160, 7, limpiar_texto_pdf(str(row["Descripción"])), 1, 1)
                ...
                try:
                    ...
                    st.error(f"Error al generar el PDF base: {e}")
                    st.stop()
                ...
                if correo_enviado:
                    ...
                    st.warning("⚠️ El PDF se generó correctamente, pero hubo un problema al enviar el correo (verifica tus Secrets). Descárgalo aquí abajo:")
                
                st.download_button("📥 Descargar Reporte Generado", data=pdf_bytes, file_name=nom_archivo, mime="application/pdf", type="primary")
