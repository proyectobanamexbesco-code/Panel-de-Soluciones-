...
def aplicar_estilos() -> None:
    st.markdown(
        """
        <style>
        .vehicular-header { text-align: center; padding: 0.5rem 0 1rem 0; }
        .vehicular-title { font-size: 2.1rem; font-weight: 800; color: #1E3A5F; margin-bottom: 0.2rem; }
        .vehicular-subtitle { font-size: 1rem; color: #5B6573; margin-bottom: 1rem; }
        .info-card {
            background: linear-gradient(135deg, #F7F9FC 0%, #EEF3F8 100%);
            border: 1px solid #D9E2EC; border-radius: 14px; padding: 16px 18px; margin-bottom: 1rem;
        }
        .info-card p { margin: 0.2rem 0; color: #334E68; font-size: 0.95rem; }
        .section-title { font-size: 1.25rem; font-weight: 700; color: #1E3A5F; margin-top: 1.5rem; margin-bottom: 0.7rem; }
        .ok-box { background-color: #E3FCEF; border: 1px solid #B7E4C7; color: #127C56; border-radius: 12px; padding: 12px 14px; margin-top: 10px; }
        .warning-box { background-color: #FFF4E5; border: 1px solid #F3D19C; color: #9A6700; border-radius: 12px; padding: 12px 14px; margin-top: 10px; }
        .footer-text { text-align: center; color: #7B8794; font-size: 0.85rem; padding-top: 1rem; }
        </style>
        """,
...
def nueva_pagina_si_necesaria(
    c: canvas.Canvas, y: int, espacio_requerido: int = 120
) -> int:
...
def escribir_linea_pdf(
    c: canvas.Canvas, etiqueta: str, valor: str, y: int
) -> int:
...
def dibujar_imagen_en_pdf(
    ...
    try:
        ...
        c.drawString(
            PDF_MARGIN_LEFT, y - 20, f"No se pudo insertar la imagen: {error}"
        )
        y -= 45
...
def crear_pdf_reporte(
    ...
    c.setFillColor(colors.black)
    texto_observaciones = (
        observaciones.strip() if observaciones.strip() else "Sin observaciones."
    )
    ...
    for nombre_seccion, uploaded_file in imagenes.items():
        if uploaded_file is not None:
            ...
            temp_img_paths.append(image_path)
    ...
    if not hay_imagenes:
        ...
        c.setFillColor(colors.black)
        c.drawString(
            PDF_MARGIN_LEFT, y, "No se adjuntaron evidencias fotográficas."
        )
        y -= 20
    ...
    # Limpieza de imágenes temporales de los uploads
...
def main() -> None:
    ...
    with col_back:
        st.page_link(
            "portal.py", label="⬅️ Volver al portal", use_container_width=True
        )
    ...
    with col_filtro_1:
        ...
        oficina_seleccionada = st.selectbox("Filtrar por oficina", oficinas)

    df_filtrado = (
        df
        if oficina_seleccionada == "Todas"
        else df[df["Oficina"] == oficina_seleccionada]
    )
    ...
    with col_filtro_2:
        ...
        if not placas:
            ...
            st.stop()
        placa_seleccionada = st.selectbox("Selecciona la placa", placas)

    # Limpiar caché de PDF si cambia la placa seleccionada
    if st.session_state.get("ultima_placa") != placa_seleccionada:
        st.session_state["ultima_placa"] = placa_seleccionada
        if "pdf_path_vehicular" in st.session_state:
            old_pdf = st.session_state["pdf_path_vehicular"]
            if os.path.exists(old_pdf):
                try:
                    os.remove(old_pdf)
                except Exception:
                    pass
            del st.session_state["pdf_path_vehicular"]
    ...
    with col_datos_1:
        kilometraje = st.text_input(
            "Kilometraje del tablero", placeholder="Ejemplo: 45,230"
        )
    with col_datos_2:
        nivel_combustible = st.selectbox(
            "Nivel de combustible",
            ["No especificado", "Vacío", "1/4", "1/2", "3/4", "Lleno"],
        )
    with col_datos_3:
        estado_general = st.selectbox(
            "Estado general del vehículo",
            ["Bueno", "Regular", "Malo", "Requiere revisión"],
        )
    ...
    secciones_opcionales = ["Asientos traseros"]
    imagenes = {}
    ...
    for i in range(0, len(secciones_obligatorias), 2):
        ...
        with col_img_1:
            ...
            imagenes[seccion] = st.file_uploader(
                f"{seccion} *", type=["jpg", "jpeg", "png"], key=f"img_{seccion}"
            )
    ...
    permitir_generar_incompleto = st.checkbox(
        "Permitir generar PDF aunque falten evidencias/datos", value=True
    )
    puede_generar = permitir_generar_incompleto or not faltantes

    if st.button(
        "📄 Generar PDF", use_container_width=True, disabled=not puede_generar
    ):
        ...
        nombre_pdf = (
            nombre_pdf.replace(" ", "_").replace("/", "_").replace("\\", "_")
        )
    ...
    if "pdf_path_vehicular" in st.session_state:
        ...
        with st.expander("📧 Envío por correo", expanded=False):
            destinatario = st.text_input(
                "Correo destinatario", placeholder="ejemplo@empresa.com"
            )
            asunto = st.text_input(
                "Asunto", value=f"Reporte vehicular - {placa_seleccionada}"
            )
            cuerpo = st.text_area(
                "Mensaje",
                value=(
